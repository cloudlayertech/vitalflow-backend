...
router.get('/trends', async (req, res, next) => {
  try {
    ...
    const healthTrend = await query(
      `SELECT date, steps, readiness_score, sleep_score, hrv_score, resting_hr, wellness_score
    ...
    const activityTrend = await query(
      `SELECT DATE_TRUNC('week', start_date) as week, COUNT(*) as activity_count,
        COALESCE(SUM(distance_meters), 0) as total_distance_m,
        COALESCE(SUM(duration_seconds), 0) as total_duration_sec,
        COALESCE(SUM(calories), 0) as total_calories,
        COALESCE(SUM(training_load), 0) as total_training_load
    ...
    const fitnessTrend = await query(
      `SELECT date, ctl, atl, tsb, weekly_tss
       FROM training_metrics
       WHERE date >= CURRENT_DATE - INTERVAL '${days} days'
       ORDER BY date ASC`
    );

    res.json({ days, health: healthTrend.rows, activities: activityTrend.rows, fitness: fitnessTrend.rows });
  } catch (err) {
    next(err);
  }
});
...
router.get('/alerts', async (req, res, next) => {
  try {
    ...
    const result = await query(
      `SELECT id, alert_type, severity, title, message, source_table, source_record_id,
        field_name, field_value, threshold_value, triggered_at, dismissed_at, expires_at
       FROM alerts
       WHERE user_id = $1 AND dismissed_at IS NULL AND (expires_at IS NULL OR expires_at > NOW())
