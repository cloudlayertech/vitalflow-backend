...
router.get('/strava/callback', async (req, res, next) => {
  try {
    ...
    await query(
      'INSERT INTO oauth_connections (user_id, provider, provider_athlete_id, access_token, refresh_token, token_expires_at, scope, sync_status) ' +
      ...
      'sync_status = $8, connected_at = NOW(), last_sync_at = NOW()',
      [userId, 'strava', String(tokenData.athlete.id), tokenData.access_token, tokenData.refresh_token, new Date(Date.now() + tokenData.expires_in * 1000), 'read,activity:read_all,profile:read_all', 'ok']
    );
...
router.get('/connections', requireAuth, async (req, res, next) => {
  try {
    const result = await query(
      'SELECT provider, scope, connected_at, last_sync_at, sync_status ' +
...
function apiGet(hostname, path, headers) {
  return new Promise((resolve, reject) => {
    const request = https.request(
      { hostname: hostname, port: 443, path: path, method: 'GET', headers: headers },
      (response) => {
        let data = '';
        response.on('data', (chunk) => { data += chunk; });
        response.on('end', () => {
      ...
      response.on('end', () => {
        try {
          ...
          if (parsed.access_token) {
            query(
              'UPDATE oauth_connections SET access_token = $1, refresh_token = $2, ' +
              'token_expires_at = $3, sync_status = $4 WHERE user_id = $5 AND provider = $6',
              [parsed.access_token, parsed.refresh_token,
                new Date(Date.now() + parsed.expires_in * 1000), 'ok', userId, 'strava']
            );
            resolve(parsed.access_token);
          } else {
            reject(new Error('Token refresh failed: ' + (parsed.message || 'unknown error')));
          }
        } catch {
...
router.post('/strava/sync', requireAuth, async (req, res, next) => {
  try {
    const conn = await query(
      'SELECT access_token, refresh_token, token_expires_at FROM oauth_connections ' +
    ...
    if (expiresAt && new Date(expiresAt) < new Date(Date.now() + 5 * 60000)) {
      logger.info('Strava sync: refreshing expired token for user ' + req.user.userId);
      try {
        accessToken = await refreshStravaToken(req.user.userId, refreshToken);
      } catch (refreshErr) {
        logger.error('Strava token refresh failed: ' + refreshErr.message);
        return res.status(401).json({
          error: 'Strava token expired and refresh failed. Please reconnect Strava.'
        });
      }
    }

    logger.info('Strava sync: fetching activities for user ' + req.user.userId);
    const after = Math.floor(Date.now() / 1000) - 90 * 86400; // 90 days
    const activities = await apiGet(
      'www.strava.com',
      '/api/v3/athlete/activities?after=' + after + '&per_page=200',
    ...
    if (!Array.isArray(activities)) {
      if (activities.message === 'Authorization Error') {
        return res.status(401).json({
          error: 'Strava authorization error. Please reconnect Strava.'
        });
      }
      logger.error(
        'Strava sync: unexpected response type: ' + typeof activities +
    ...
    for (let i = 0; i < activities.length; i++) {
      ...
      await query(
        ...
        'start_date, duration_seconds, distance_meters, training_load, calories, raw_data, created_at) ' +
    ...
    await query(
      'UPDATE oauth_connections SET last_sync_at = NOW(), sync_status = $1 ' +
      'WHERE user_id = $2 AND provider = $3',
      ['ok', req.user.userId, 'strava']
    );
    res.json({ imported: imported, total: activities.length });
  } catch (err) {
    logger.error('Strava sync: ' + err.message);
    next(err);
  }
});
...
router.post('/oura/sync', requireAuth, async (req, res, next) => {
  try {
    ...
    const conn = await query(
      'SELECT access_token FROM oauth_connections WHERE user_id = $1 AND provider = $2',
      [req.user.userId, 'oura']
    );
    if (conn.rows.length === 0) {
      return res.status(404).json({ error: 'Oura not connected' });
    }
    ...
    if (sleepDetail && Array.isArray(sleepDetail.data)) {
      for (let i = 0; i < sleepDetail.data.length; i++) {
        const s = sleepDetail.data[i];
    ...
    await query(
      'UPDATE oauth_connections SET last_sync_at = NOW(), sync_status = $1 ' +
      'WHERE user_id = $2 AND provider = $3',
      ['ok', req.user.userId, 'oura']
    );
    res.json({
      imported: imported,
      data: {
        sleep: dailySleep.data ? dailySleep.data.length : 0,
        readiness: readiness.data ? readiness.data.length : 0,
        activity: activity.data ? activity.data.length : 0,
      },
    });
  } catch (err) {
    logger.error('Oura sync: ' + err.message);
    next(err);
  }
});
...
router.get('/debug', requireAuth, async (req, res) => {
  const conns = await query(
    'SELECT provider, scope, sync_status, last_sync_at FROM oauth_connections WHERE user_id = $1',
    [req.user.userId]
  );
...
router.get('/oura/debug', requireAuth, async (req, res) => {
  try {
    ...
    const hrvSamples = [];
    if (sleepDetail.data) {
      for (let i = 0; i < Math.min(3, sleepDetail.data.length); i++) {
        const s = sleepDetail.data[i];
    ...
    res.json({
      dailySleep_count: dailySleep.data ? dailySleep.data.length : 0,
      sleepDetail_count: sleepDetail.data ? sleepDetail.data.length : 0,
      readiness_count: readiness.data ? readiness.data.length : 0,
      hrv_samples: hrvSamples,
      first_daily_sleep: dailySleep.data ? dailySleep.data[0] : null,
      first_sleep_detail: sleepDetail.data && sleepDetail.data[0] ? {
        day: sleepDetail.data[0].day,
        average_hrv: sleepDetail.data[0].average_hrv,
        average_heart_rate: sleepDetail.data[0].average_heart_rate,
      } : null,
    });
  } catch (err) {
...
router.get('/strava/debug', requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      } : { error: athlete.message || 'unknown' },
      activities_test: Array.isArray(activities) ? {
        count: activities.length,
        first: activities[0] ? activities[0].name : null,
      } : { error: activities.message || activities },
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
