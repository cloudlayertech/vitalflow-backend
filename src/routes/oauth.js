...
// HashRouter format: redirect to /#/path?query instead of /path?query
...
// Reads token from req.query.token, verifies JWT via requireAuth, redirects to Strava
...
router.get('/strava/callback', async (req, res, next) => {
  try {
    ...
    // Exchange code for token via https.request with JSON body
    const tokenData = await stravaTokenRequest(code);

    // 8 columns, 8 values ($1-$8)
    await query(
...
router.post('/strava/disconnect', requireAuth, async (req, res, next) => {
  try {
    ...
    if (result.rows.length === 0) {
      logger.warn(`Strava disconnect: no connection found for user ${req.user.userId}`);
    }
...
// Reads token from req.query.token, verifies JWT inline, redirects to Oura
...
router.get('/oura/callback', async (req, res, next) => {
  try {
    ...
    // Exchange code for token via https.request with form-urlencoded body
...
// POST /api/oauth/oura/disconnect
router.post('/oura/disconnect', requireAuth, async (req, res, next) => {
  try {
    ...
    if (result.rows.length === 0) {
      logger.warn(`Oura disconnect: no connection found for user ${req.user.userId}`);
    }

    logger.info(`Oura disconnected for user ${req.user.userId}`);
...
// Helper: exchange Oura code for tokens using form-urlencoded body
...
// Helper: exchange Strava code for tokens using JSON body
...
// Auto-create health_data table if missing
  ...
  try {
    ...
    if (readiness && Array.isArray(readiness.data)) {
      for (const d of readiness.data) {
        ...
        await query('INSERT INTO health_data (user_id, provider, date, metric_type, value, raw_data, created_at) VALUES ($1, $2, $3, $4, $5, $6, NOW()) ON CONFLICT (user_id, provider, date, metric_type) DO UPDATE SET value = EXCLUDED.value, raw_data = EXCLUDED.raw_data, updated_at = NOW()', [req.user.userId, 'oura', d.day, 'hrv', d.hrv_balance || 0, JSON.stringify(d)]);
        imported += 2;
      }
    }
